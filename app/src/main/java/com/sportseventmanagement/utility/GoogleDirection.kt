...
@SuppressLint("NewApi")
class GoogleDirection(context: Context?) {
    ...
    fun request(start: LatLng, end: LatLng, mode: String): String {
        val url = ("https://maps.googleapis.com/maps/api/directions/xml?"
                + "origin=${start.latitude},${start.longitude}&destination=${end.latitude},${end.longitude}&sensor=false&units=metric&mode=$mode&key=AIzaSyD3h20CTKpHUM_zB1FcTK8irTowxPZaghI")
        if (isLogging) Log.i("GoogleDirection", "URL : $url")
        RequestTask().execute(*arrayOf(url))
        return url
    }

    private inner class RequestTask :
        AsyncTask<String?, Void?, Document?>() {


        override fun onPostExecute(doc: Document?) {
            super.onPostExecute(doc)
//            if (mDirectionListener != null) mDirectionListener!!.onResponse(
//                getStatus(doc!!),
//                doc,
...
//            if (isLogging) Log.i("GoogleDirection", "Status : " + node1.textContent)
...
//                val postReq = object : StringRequest(Request.Method.POST, url[0],
...
//                requestQueue!!.add(postReq)
            ...
            //    Log.e("Anas", "${url[0]} url from async task")
                val httpPost = HttpPost(url[0])
                val response: org.apache.http.HttpResponse? =
                    httpClient.execute(httpPost, localContext)
                val `in`: InputStream = response!!.entity.content
                Log.d("Anas","${response!!.params.toString()}")
                val builder = DocumentBuilderFactory.newInstance().newDocumentBuilder()
//
    ...
    fun getDurationText(doc: Document): Array<String?> {
        ...
        for (i in 0 until nl1.length - 1) {
            ...
            if (isLogging) Log.i("GoogleDirection", "DurationText : " + node2.textContent)
        }
        return arr_str
    }
    ...
    fun getDurationValue(doc: Document): IntArray {
        ...
        for (i in 0 until nl1.length - 1) {
            ...
            if (isLogging) Log.i("GoogleDirection", "Duration : " + node2.textContent)
        }
        return arr_int
    }
    ...
    fun getTotalDurationText(doc: Document): String {
        ...
        val nl2 = node1.childNodes
        val node2 = nl2.item(getNodeIndex(nl2, "text"))
        if (isLogging) Log.i("GoogleDirection", "TotalDuration : " + node2.textContent)
    ...
    fun getStartAddress(doc: Document): String {
        ...
        val node1 = nl1.item(0)
        if (isLogging) Log.i("GoogleDirection", "StartAddress : " + node1.textContent)
        return node1.textContent
    }
    ...
    fun getCopyRights(doc: Document): String {
        ...
        val node1 = nl1.item(0)
        if (isLogging) Log.i("GoogleDirection", "CopyRights : " + node1.textContent)
        return node1.textContent
    }
            ...
            if (animateMarkerPosition!!.latitude == endPosition!!.latitude
                ...
                if (step == animatePositionList!!.size - 2) {
                    ...
                    if (flatMarker && step + 3 < animatePositionList!!.size - 1) {
                        val rotation = getBearing(
                            animateMarkerPosition,
                            animatePositionList!![step + 3]
                        ) + 180
                        animateMarker!!.rotation = rotation
                    }
                    if (mAnimateListener != null) mAnimateListener!!.onProgress(
    ...
    interface OnDirectionResponseListener {
        fun onResponse(status: String?, doc: Document?, gd: GoogleDirection?)
    }
    ...
    private fun getNewPosition(begin: LatLng?, end: LatLng?): LatLng? {
        ...
        return if (dis >= animateDistance) {
            ...
            if (begin.latitude <= end.latitude && begin.longitude <= end.longitude) angle =
                Math.toDegrees(
                    Math.atan(lng / lat)
                ) else if (begin.latitude > end.latitude && begin.longitude <= end.longitude) angle =
                90 - Math.toDegrees(
                    Math.atan(lng / lat)
                ) + 90 else if (begin.latitude > end.latitude && begin.longitude > end.longitude) angle =
                Math.toDegrees(
                    Math.atan(lng / lat)
                ) + 180 else if (begin.latitude <= end.latitude && begin.longitude > end.longitude) angle =
                90 - Math.toDegrees(
                    Math.atan(lng / lat)
                ) + 270
